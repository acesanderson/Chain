...
#         self._connection = None  # Remove self.cursor, add this
#         self._create_table()
#
#     @property
#     def connection(self):
...
#         """
#         try:
#             cursor = self.connection.cursor()  # Use self.connection instead of self.connection.cursor
#             cursor.execute("SELECT response_data FROM cache WHERE cache_key = ?", (cache_key,))
...
#     def clear_cache(self):
#         """Clear all cached responses."""
#         self.connection.cursor().execute("DELETE FROM cache")
#         self.connection.commit()
#
#     def retrieve_cached_requests(self):
#         """
...
#             Dict with cache statistics
#         """
#         self.connection.cursor().execute("SELECT COUNT(*) FROM cache")
#         total_entries = self.connection.cursor().fetchone()[0]
#
#         self.connection.cursor().execute("SELECT SUM(LENGTH(response_data)) FROM cache")
#         total_size = self.connection.cursor().fetchone()[0] or 0
#
#         return {
#             "total_entries": total_entries,
#             "total_size_bytes": total_size,
...
#             cache_key: Key of the entry to delete
#         """
#         self.connection.cursor().execute("DELETE FROM cache WHERE cache_key = ?", (cache_key,))
#         self.connection.commit()
#
#     def cleanup_old_entries(self, days: int = 30):
#         """
