...
class ChainCache:
                ...
                if content_class_name:
                    ...
                    if cls is None:
                        if content_class_name == "str":
                            ...
                            cls = dict
                        elif content_class_name == "ChatCompletion": # Specific OpenAI type
                            import openai.types.chat.chat_completion as oc
                            cls = oc.ChatCompletion
                        elif content_class_name == "Completion": # Specific OpenAI type for /v1/completions
                        ...
                        elif content_class_name == "Message": # Assumes anthropic.types.messages.Message
    ...
    def retrieve_cached_requests(self):
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("SELECT cache_key, content, content_class_name FROM cache")
...
def check_cache_and_query(model_instance: Any, params: Any, query_func: Callable) -> Any:
...
async def check_cache_and_query_async(model_instance: Any, params: Any, query_func: Callable) -> Any:
    # Do not cache if streaming is requested, as stream objects are not serializable.
    if params.stream:
    ...
    if model_instance._chain_cache:
        cache_key = params.generate_cache_key()
        cached_result = model_instance._chain_cache.retrieve_from_cache(cache_key) # retrieve_from_cache is sync
        ...
        if cached_result is not None:
            ...
            if not is_stream_chunk: # Only store if it's not a stream chunk
                model_instance._chain_cache.store_in_cache(cache_key, result) # store_in_cache is sync
            return result
    else:
