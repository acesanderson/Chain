...
class Model:
	...
	def _validate_model(cls, model: str) -> str:
		...
		if model in aliases.keys():
			model = aliases[model]
		elif model in list(itertools.chain.from_iterable(cls.models.values())):   # any other model we support (flattened the list)
			model = model
		else:
	...
	def _get_client(cls, client_type: str):
		...
		if client_type[0] not in cls._clients:
			try:
				module = importlib.import_module(f'clients.{client_type[0].lower()}_client')
	...
	def query(self, input: 'str | list', verbose: bool = True, pydantic_model: 'BaseModel' = None) -> 'BaseModel | str':
		if verbose:
			print(f"Model: {self.model}   Query: " + self.pretty(str(input)))
		return self._client.query(self.model, input, pydantic_model)

	async def query_async(self, input: 'str | list', verbose: bool = True, pydantic_model: 'BaseModel' = None) -> 'BaseModel | str':
