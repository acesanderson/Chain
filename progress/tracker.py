    ...
    def emit_concurrent_start(self):
        ...
        if hasattr(self.handler, 'handle_concurrent_start'):
            ...
            if hasattr(self.handler, 'show_spinner'):
                self.handler.show_spinner("concurrent", f"Running {self.total} concurrent requests...")
            else:
                self.handler.emit_started("concurrent", f"Starting: {self.total} concurrent requests")
    ...
    def emit_concurrent_complete(self):
        ...
        if hasattr(self.handler, 'handle_concurrent_complete'):
            ...
            if hasattr(self.handler, 'show_complete'):
                self.handler.show_complete("concurrent", f"All requests complete: {successful}/{self.total} successful", duration)
            else:
                self.handler.emit_complete("concurrent", f"All requests complete: {successful}/{self.total} successful", duration)
