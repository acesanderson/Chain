...
class RichDisplayParamsMixin(RichDisplayMixin):
    ...
    def _format_params_detailed_rich(self) -> "Panel":
        ...
        timestamp = datetime.now().strftime("%H:%M:%S")
        return Panel(table, title=f"► REQUEST {getattr(self, 'model', 'unknown')} (Detailed)", 
                    title_align="left", subtitle=f"[dim]{timestamp}[/dim]", subtitle_align="right")
    ...
    def _format_params_complete_rich(self) -> "Panel":
        ...
        timestamp = datetime.now().strftime("%H:%M:%S")
        return Panel(table, title=f"► REQUEST {getattr(self, 'model', 'unknown')} (Complete)", 
                    title_align="left", subtitle=f"[dim]{timestamp}[/dim]", subtitle_align="right")
    ...
    def _format_params_debug_rich(self) -> "Panel":
        ...
        if hasattr(self, 'messages') and self.messages:
            ...
            debug_data["messages"] = [{"role": "user", "content": str(self.query_input)}]
            ...
            if hasattr(self, attr):
                ...
                if value is not None:
                    ...
                    if hasattr(value, 'to_json_serializable'):
                        ...
                        debug_data[attr] = str(value) if not isinstance(value, (int, float, bool)) else value
        ...
        return Panel(syntax, title="🐛 FULL DEBUG REQUEST", 
                    title_align="left", subtitle=f"[dim]{datetime.now().strftime('%H:%M:%S')}[/dim]", 
...
class PlainDisplayParamsMixin(PlainDisplayMixin):
    ...
    def _format_params_detailed_plain(self) -> str:
        ...
        # Header
        lines.append(f"[{timestamp}] REQUEST {getattr(self, 'model', 'unknown')} (Detailed)")
    ...
    def _format_params_complete_plain(self) -> str:
        ...
        # Header
        lines.append(f"[{timestamp}] REQUEST {getattr(self, 'model', 'unknown')} (Complete)")
...
class RichDisplayResponseMixin(RichDisplayMixin):
    ...
    def _format_response_detailed_rich(self) -> "Panel":
        ...
        if hasattr(self, 'params') and self.params:
            if hasattr(self.params, 'temperature') and self.params.temperature is not None:
                metadata_lines.append(f"Temperature: {self.params.temperature}")
        
        if metadata_lines:
            content.append(" • ".join(metadata_lines), style="dim")
        
        timestamp = datetime.now().strftime("%H:%M:%S")
        return Panel(content, title=f"✓ RESPONSE {duration:.1f}s (Detailed)", 
                    title_align="left", subtitle=f"[dim]{timestamp}[/dim]", subtitle_align="right")
    ...
    def _format_response_complete_rich(self) -> "Panel":
        ...
        if hasattr(self, 'params') and self.params:
            if hasattr(self.params, 'temperature') and self.params.temperature is not None:
                metadata_lines.append(f"Temperature: {self.params.temperature}")
            if hasattr(self.params, 'parser') and self.params.parser:
                parser_name = getattr(self.params.parser, 'pydantic_model', {})
                if hasattr(parser_name, '__name__'):
                    metadata_lines.append(f"Parser: {parser_name.__name__}")
        ...
        timestamp = datetime.now().strftime("%H:%M:%S")
        return Panel(content, title=f"✓ RESPONSE {duration:.1f}s (Complete)", 
                    title_align="left", subtitle=f"[dim]{timestamp}[/dim]", subtitle_align="right")
    ...
    def _format_response_debug_rich(self) -> "Panel":
        ...
        # Create debug dictionary with all response information
        debug_data = {
            "type": "Response",
            "timestamp": datetime.now().isoformat(),
            "duration": duration,
        ...
        # Add content (handle different types)
        content = getattr(self, 'content', None)
        if content is not None:
            try:
                # Try to serialize pydantic models
                if hasattr(content, 'model_dump'):
                    debug_data["content"] = content.model_dump()
                elif hasattr(content, '__dict__'):
                    debug_data["content"] = content.__dict__
                else:
                    debug_data["content"] = str(content)
            except:
                debug_data["content"] = str(content)
        
        # Add messages if available
        if hasattr(self, 'messages') and self.messages:
            debug_data["messages"] = safe_json_serialize(self.messages)
        
        # Add params if available
        if hasattr(self, 'params') and self.params:
            debug_data["params"] = safe_json_serialize(self.params)
...
class PlainDisplayResponseMixin(PlainDisplayMixin):
    ...
    def _format_response_detailed_plain(self) -> str:
        ...
        # Header
        lines.append(f"[{timestamp}] RESPONSE {duration:.1f}s (Detailed)")
    ...
    def _format_response_complete_plain(self) -> str:
        ...
        # Header
        lines.append(f"[{timestamp}] RESPONSE {duration:.1f}s (Complete)")
        
        # Show full response content
        response_content = str(getattr(self, 'content', 'No content'))
        lines.append(f"Complete Content: {response_content}")
    ...
    def _format_response_debug_plain(self) -> str:
        ...
        # Header
        lines.append(f"[{timestamp}] DEBUG RESPONSE {duration:.1f}s")
        ...
        # Format as JSON
        json_content = json.dumps(safe_debug_data, indent=2, ensure_ascii=False)
        lines.append("Complete Response Object:")
        lines.append(json_content)
...
class RichDisplayChainErrorMixin(RichDisplayMixin):
    ...
    def _format_error_detailed_rich(self) -> "Panel":
        ...
        if hasattr(self, 'detail') and self.detail:
            ...
            if self.detail.request_params:
                content.append(f"\nRequest had {len(self.detail.request_params)} parameters", style="dim")
        
        timestamp = datetime.now().strftime("%H:%M:%S")
        return Panel(content, title="✗ ERROR (Detailed)", title_align="left", 
                    subtitle=f"[dim]{timestamp}[/dim]", subtitle_align="right", border_style="red")
    ...
    def _format_error_complete_rich(self) -> "Panel":
        ...
        if hasattr(self, 'detail') and self.detail:
            ...
            if self.detail.request_params:
                content.append(f"Request Parameters: {len(self.detail.request_params)} items\n", style="dim")
            ...
            if self.detail.stack_trace:
                ...
                if len(stack_lines) > 10:
                    shown_lines = stack_lines[:3] + ['  ... (truncated) ...'] + stack_lines[-3:]
                else:
        ...
        timestamp = datetime.now().strftime("%H:%M:%S")
        return Panel(content, title="✗ ERROR (Complete)", title_align="left", 
                    subtitle=f"[dim]{timestamp}[/dim]", subtitle_align="right", border_style="red")
    ...
    def _format_error_debug_rich(self) -> "Panel":
        ...
        if hasattr(self, 'info'):
            debug_data["info"] = {
                ...
                "timestamp": self.info.timestamp.isoformat() if hasattr(self.info.timestamp, 'isoformat') else str(self.info.timestamp)
            }
        ...
        return Panel(syntax, title="🐛 FULL DEBUG ERROR", 
                    title_align="left", subtitle=f"[dim]{datetime.now().strftime('%H:%M:%S')}[/dim]", 
                    subtitle_align="right", border_style="red")
...
class PlainDisplayChainErrorMixin(PlainDisplayMixin):
    ...
    def _format_error_detailed_plain(self) -> str:
        ...
        if hasattr(self, 'detail') and self.detail:
            ...
            if self.detail.request_params:
                lines.append(f"Request had {len(self.detail.request_params)} parameters")
    ...
    def _format_error_complete_plain(self) -> str:
        ...
        if hasattr(self, 'detail') and self.detail:
            ...
            if self.detail.request_params:
                lines.append(f"Request Parameters: {len(self.detail.request_params)} items")
