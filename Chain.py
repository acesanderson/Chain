...
from jinja2 import (
    ...
    StrictUndefined,
    Template,
)  # we use jinja2 for prompt templates
...
class Chain:
    ...
    ollama_context_sizes = defaultdict(
        lambda: 4096
    )  # DefaultDict so that any model not listed here defaults to 2048
    ...
    def standard_repr(object) -> str:
        ...
        """
        attributes = ", ".join(
            [f"{k}={repr(v)[:50]}" for k, v in object.__dict__.items()]
        )
    ...
    def __init__(self, prompt=None, model=None, parser=None):
        if prompt is None:  # if inputs are empty, use the defaults from Model.examples
            ...
            prompt = Prompt(
                prompt
            )  # if prompt is a string, turn it into a Prompt object <-- for fast iteration
        if model is None:
        ...
        self.output_schema = {
            "result"
        }  # in the future, we'll allow users to define this, for chaining purposes

    def __repr__(self):
        return Chain.standard_repr(self)

    def create_messages(
        system_prompt=examples["system_prompt_example"], input_variables=None
    ) -> list[dict]:
        """
    ...
    def run(self, input_variables: Union[str, dict] = None, verbose=True, messages=[]):
        ...
        if messages:
            ...
            result = self.run_completion(
                prompt=prompt, model=self.model.model, verbose=verbose
            )
        return result
...
class Model:
    ...
    _clients = (
        {}
    )  # This is where we store our clients; we don't want to keep creating them. (lazy load implementation)
    ...
    def __init__(self, model=Chain.examples["model_example"]):
        ...
        self.example_query = Prompt(Chain.examples["prompt_example"]).render(
            Chain.examples["run_example"]
        )
    ...
    def _validate_model(self, model: str) -> str:
        ...
        if model == "claude":
            self.model = (
                "claude-3-5-sonnet-20240620"  # newest claude model as of 6/21/2024
            )
        elif model == "gpt":
            self.model = (
                "gpt-4o"  # defaulting to the cheap strong model they just announced
            )
        elif model == "gpt3":
            ...
            itertools.chain.from_iterable(Chain.models.values())
    ...
    def _get_client(cls, client_name):
        ...
        if client_name not in cls._clients:
            if client_name == "openai":
                ...
                cls._clients[client_name] = instructor.from_openai(
                    OpenAI(api_key=api_keys["OPENAI_API_KEY"])
                )
            elif client_name == "anthropic":
                Anthropic = importlib.import_module("anthropic").Anthropic
                instructor = importlib.import_module("instructor")
                cls._clients[client_name] = instructor.from_anthropic(
                    Anthropic(api_key=api_keys["ANTHROPIC_API_KEY"])
                )
    ...
    def query(
        self, input: Union[str, list], verbose: bool = True, pydantic_model=None
    ) -> Union[BaseModel, str, List]:
...
What is't but to be nothing else but mad? / But let that go.
    ...
    async def query_openai_async(
        ...
        if isinstance(prompt, str):
            ...
            raise ValueError(
                f"Prompt not recognized as a valid prompt type: {type(prompt)}: {prompt}"
            )
        ...
        response = await async_client_openai.chat.completions.create(
            model=model, response_model=pydantic_model, messages=prompt
        )
        ...
        if pydantic_model:
            ...
            if hasattr(response, "choices"):
                ...
                )  # Debugging line to inspect the structure
    ...
    async def run_multiple_extracts(
        ...
        for i, p in enumerate(prompts, start=1):
            ...
            if model == "gpt":
                tasks.append(
                    self.query_openai_async(
                        p, pydantic_model=pydantic_model, request_num=i, verbose=verbose
                    )
                )
            elif model == "claude":
                tasks.append(
                    self.query_anthropic_async(
                        p, pydantic_model=pydantic_model, request_num=i, verbose=verbose
                    )
                )
        ...
        (
            print(
                f"All {len(prompts)} tasks completed in {end_time - start_time:.2f} seconds."
            )
            if verbose
            else None
        )
    ...
    def run_async(
        ...
        if throttle > 0:
            if len(prompts) > throttle:
                raise ValueError(
                    "You've requested more than 50 prompts; throwing an error to spare you bank account."
                )
        results = asyncio.run(
            self.run_multiple_extracts(
                prompts, pydantic_model=pydantic_model, verbose=verbose, model=model
            )
        )
        return results
...
class Chat:
    ...
    def __init__(
        self,
        model=Chain.examples["model_example"],
        system_prompt=Chain.examples["system_prompt_example"],
    ):
    ...
    def chat(self, messages: Messages = []):
        ...
        while True:
            ...
            if self.model.model in Chain.models["anthropic"]:
                if messages[0]["role"] == "system":
                    ...
                    messages.append(
                        {
                            "role": "assistant",
                            "content": "OK, I will follow that as my system message for this conversation.",
                        }
                    )
            # grab user input
            ...
            if (
                user_input[:12] == "/set system "
            ):  # because match/case doesn't do wildcards or regex.
                new_system_prompt = user_input[12:]
                user_input = "/set system"
            if (
                user_input[:11] == "/set model "
            ):  # because match/case doesn't do wildcards or regex.
                ...
                case "/set model":
                    if not new_model:
                        ...
                        try:
                            ...
                            print(
                                f"Model set to {new_model}. It may take a while to load after your next message."
                            )
                        except ValueError:
